(function(){
'use strict';
const VERSION='专科版｜V1.1.69 专科批次规则版';
...
const specialTypeFacetGroups=[
  {title:'重点特殊类型',items:['定向军士','定向军士男','定向军士女','3+2','5+0','联合培养','中外合作','定向医学','定向医学生','司法警察男','司法警察女','石邮连云港','石邮扬州','石邮淮安','石邮镇江','石邮南通','石邮宿迁']},
  {title:'合作与培养模式',items:['联合培养','高收费','境外/国际培养','校企合作/产教融合']},
  {title:'政策与特殊招生',items:['军校','公安','航海/飞行','专项计划','其他院校提前批','强基计划','综评A','综评B','定向/军士','定向/公费/优师','民族班/预科','军警航海']},
  {title:'特殊班型',items:['实验班/拔尖班','双学位/本博硕博']}
];
const SPECIALTY_BATCH_EARLY='specialty:early';
const SPECIALTY_BATCH_NORMAL='specialty:normal';
const SPECIALTY_BATCH_FILTERS=[
  {value:SPECIALTY_BATCH_EARLY,label:'专科提前批',summary:'只看定向军士'},
  {value:SPECIALTY_BATCH_NORMAL,label:'专科普通批',summary:'联合培养 / 定向医学 / 普通专科'}
];
...
let DETAILS=window.MAJOR_DETAILS||window.DETAILS||{};
...
const VOLUNTEER_TOGGLE_POSITION_KEY='js-plan-volunteer-toggle-position-v1';
...
function groupFactText(s,g){
  const parts=[
    s?.name,s?.level,s?.batch,s?.subject,s?.province,s?.city,
    g?.batch,g?.groupName,g?.displayCode,g?.rawGroupName,g?.majorSummary,g?.remark,g?.rawGroupMajors,
    (Array.isArray(g?.tags)?g.tags:[]).join(' '),
    (Array.isArray(g?.majorClasses)?g.majorClasses:[]).join(' ')
  ];
  (Array.isArray(g?.majors)?g.majors:[]).forEach(m=>{
    const d=DETAILS[m.key]||{};
    parts.push(
      m.name,m.baseName,m.remark,m.majorClass,m.discipline,m.tuition,m.duration,m.subjectRequirement,
      d.majorFullName,d.majorRemark,d.rawMajorLine,d.rawGroupMajors,d.tuition,d.duration,d.schoolTags,d.schoolLevel,
      d.recruitChapter,d.admissionRule,d.foreignCoopDurationAbroad,d.foreignCoopOtherInfo
    );
  });
  return String(parts.filter(Boolean).join(' '));
}
function isDirectedSergeantGroup(s,g){
  return /定向培养军士|定向军士|军士生|定向培养士官|士官/.test(groupFactText(s,g));
}
function specialtyBatchKind(s,g){
  return isDirectedSergeantGroup(s,g)?SPECIALTY_BATCH_EARLY:SPECIALTY_BATCH_NORMAL;
}
function specialtyBatchLabel(s,g){
  return specialtyBatchKind(s,g)===SPECIALTY_BATCH_EARLY?'专科提前批':'专科普通批';
}
function specialtyBatchSummaryLabel(s,g){
  return specialtyBatchKind(s,g)===SPECIALTY_BATCH_EARLY?'专科提前批｜定向军士':'专科普通批';
}
function isSpecialtyBatchFilterValue(value){
  return value===SPECIALTY_BATCH_EARLY||value===SPECIALTY_BATCH_NORMAL;
}
function groupTrainingModeValues(s,g){
  const vals=new Set(groupSegmentedModeValues(g));
  const text=groupFactText(s,g);
  if(/联合培养|高职院校联合|本科与高职|分段培养|贯通培养|协同培养|联合办学|联合学院/.test(text))vals.add('联合培养');
  if(/定向医学|定向医学生|免费医学生|免费医学定向|农村订单定向医学生|订单定向医学生/.test(text))vals.add('定向医学');
  const order={'3+2':1,'4+0':2,'5+0':3,'分段培养':4,'联合培养':5,'定向医学':6};
  return Array.from(vals).sort((a,b)=>(order[a]||99)-(order[b]||99)||String(a).localeCompare(String(b),'zh-Hans-CN'));
}
function specialtyBatchBadgesHTML(s,g){
  const batch=specialtyBatchKind(s,g);
  const className=batch===SPECIALTY_BATCH_EARLY?'red':'green';
  const modes=groupTrainingModeValues(s,g).filter(v=>v!=='分段培养');
  return `<span class="tag ${className}">${esc(specialtyBatchSummaryLabel(s,g))}</span>`+modes.map(v=>`<span class="tag">${esc(v)}</span>`).join('');
}
function groupMatchesSpecialtyBatch(s,g){
  if(state.batch===SPECIALTY_BATCH_EARLY)return isDirectedSergeantGroup(s,g);
  if(state.batch===SPECIALTY_BATCH_NORMAL)return !isDirectedSergeantGroup(s,g);
  return true;
}
function specialtyBatchOptionCounts(){
  const counts=new Map(SPECIALTY_BATCH_FILTERS.map(def=>[def.value,0]));
  DB.forEach(s=>(s.groups||[]).forEach(g=>{
    const key=specialtyBatchKind(s,g);
    counts.set(key,(counts.get(key)||0)+1);
  }));
  return counts;
}
function schoolSpecialtyBatchSummary(s,groups=null){
  const arr=(groups&&groups.length?groups:s?.groups)||[];
  const labels=Array.from(new Set(arr.map(g=>specialtyBatchLabel(s,g)))).filter(Boolean);
  return labels.length?labels.join('/'):(s?.batch||'专科批');
}
...
function studentTopSummary(student){
  ...
  const rank=studentRank(student);
  if(score!==null)parts.push(`${score}分`);
  if(rank!==null)parts.push(`位次${rank}`);
  const codes=studentMedicalCodes(student);
...
function setScoreFilterFromStudent(student){
  const score=dbNumber(studentScore(student));
...
function renderStudentContextBar(){
  ...
  el.hidden=false;
  el.innerHTML=`<div class="student-context-main"><b>当前学生：${esc(currentStudent.name||'未命名')}</b><span>${esc(stageLabel(currentStudent.stage))}</span><span>${esc(studentSubjectSummary(currentStudent))}</span><span>${esc(studentScore(currentStudent)??'—')}分</span><span>位次 ${esc(studentRank(currentStudent)??'—')}</span><span>${codes.length?'体检 '+esc(codes.join('/')):'体检未填'}</span>${cities.length?`<span>目标城市 ${esc(cities.join('、'))}</span>`:''}</div><div class="student-context-note">已自动同步：科类筛选、选科要求、目标分区间、体检受限。选科不符仍限制选择；体检受限只做警告确认，不再直接禁选。</div>`;
}
...
function cooperationSuffix(g){
  ...
  if(/联合培养/.test(text)||groupSegmentedModeValues(g).length)return '（联合培养）';
  return '';
}
...
function qualificationRiskForMajor(s,g,m){
  if(!window.QualificationRisk)return {level:'pass',tone:'green',label:'资格通过',issues:[],summary:''};
  return window.QualificationRisk.evaluate({student:currentStudent,school:s,group:g,major:m,details:detailOf(m)});
}
function qualificationRiskLabelHTML(meta){
  if(!meta||!Array.isArray(meta.issues)||!meta.issues.length)return '';
  const tone=meta.tone||'yellow';
  return `<span class="qualification-risk-label qualification-${esc(tone)}" title="${esc(meta.summary||meta.label||'资格提醒')}">${esc(meta.label||'资格提醒')}</span>`;
}
function qualificationRiskExportText(meta){
  if(!meta||!Array.isArray(meta.issues)||!meta.issues.length)return '';
...
function groupQualificationBadge(s,g){
  ...
  const rank=window.QualificationRisk.LEVEL_RANK||{};
  const worst=metas.reduce((a,b)=>(rank[b.level]||0)>(rank[a.level]||0)?b:a,metas[0]);
  const title=metas.slice(0,10).map(x=>x.summary).filter(Boolean).join('；');
  const tone=worst.tone||'yellow';
  return `<span class="badge qualification-group-badge qualification-${esc(tone)}" title="${esc(title)}">${worst.level==='block'?'资格风险':'资格提醒'} ${metas.length}</span>`;
}
...
function groupMedicalBadge(g){
  ...
  if(!x)return '';
  return `<span class="badge red medical-group-badge" title="当前体检代码下，本专业组有 ${x.count}/${x.total} 个专业触发风险提醒；不再强制禁选，请结合学校招生章程核对。">${x.all?'体检全限报':'体检限报'} ${x.count}/${x.total}</span>`;
}
...
function renderMedicalVolunteerNotice(){
  ...
  el.innerHTML=`<b>体检受限已启用：</b>代码 ${esc(arr.join('、'))}。受限专业会显示警告并要求确认；如已核对院校章程仍可继续选择。`;
}
...
function majorMetaBadgesHTML(m,standardDuration='三'){
  const d=detailOf(m);
...
function segmentedModeValuesFromText(text){
  const s=String(text||'');
  const vals=new Set();
  const has3=/[3三]\s*[+＋]\s*[2二]/.test(s)||/学习三年[^。；;]*转入[^。；;]*学习两年/.test(s)||/前三年[^。；;]*高职[^。；;]*后两年[^。；;]*本科/.test(s);
  const has4=/[4四]\s*[+＋]\s*[0零]/.test(s);
  const has5=/[5五]\s*[+＋]\s*[0零]/.test(s)||/学习五年[^。；;]*前三年[^。；;]*后两年[^。；;]*(?:高职院校内|继续在高职)/.test(s);
  if(has3)vals.add('3+2');
  if(has4)vals.add('4+0');
  if(has5)vals.add('5+0');
  if(!vals.size&&/分段培养|贯通培养|高本贯通|本科.*高职|高职.*本科/.test(s))vals.add('分段培养');
  return sortSegmentedModes(Array.from(vals));
}
function sortSegmentedModes(values){
  const order={'3+2':1,'4+0':2,'5+0':3,'分段培养':9};
  return Array.from(new Set(values||[])).sort((a,b)=>(order[a]||99)-(order[b]||99)||String(a).localeCompare(String(b),'zh-Hans-CN'));
}
function majorSegmentedModeValues(m,g=null){
  const d=detailOf(m);
  const vals=new Set(segmentedModeValuesFromText([
    m?.name,m?.baseName,m?.remark,m?.duration,d.majorFullName,d.majorRemark,d.schoolTags,d.recruitChapter,d.admissionRule
  ].filter(Boolean).join(' ')));
  if(g&&vals.size===1&&vals.has('分段培养')){
    groupSegmentedModeValues(g).filter(v=>v!=='分段培养').forEach(v=>vals.add(v));
    if(vals.size>1)vals.delete('分段培养');
  }
  return sortSegmentedModes(Array.from(vals));
}
function groupSegmentedModeValues(g){
  const vals=new Set(segmentedModeValuesFromText([
    g?.remark,(g?.tags||[]).join(' '),g?.majorSummary,g?.rawGroupMajors
  ].filter(Boolean).join(' ')));
  (g?.majors||[]).forEach(m=>majorSegmentedModeValues(m,null).forEach(v=>vals.add(v)));
  if(vals.size>1&&vals.has('分段培养'))vals.delete('分段培养');
  return sortSegmentedModes(Array.from(vals));
}
function segmentedModeBadgesHTML(values,type='group'){
  const modes=Array.isArray(values)?values:[];
  return modes.map(v=>{
    const cls=v==='5+0'?'five':v==='4+0'?'four':v==='3+2'?'three':'general';
    const text=v==='分段培养'?'分段培养':type==='major'?`${v}分段`:`${v} 分段培养`;
    return `<span class="segmented-mode-badge segmented-mode-${cls}" title="专科分段培养模式：${esc(v)}">${esc(text)}</span>`;
  }).join('');
}
...
function currentAdmissionBandScore(){
  const studentScore=num(currentStudent?.score);
  if(studentScore!==null)return studentScore;
  return num(state.scoreRange?.target);
}
function groupAdmissionHistory(g){
  return [
    {year:'2025',score:num(g.score25),rank:num(g.rank25)},
    {year:'2024',score:num(g.score24),rank:num(g.rank24)},
    {year:'2023',score:num(g.score23),rank:num(g.rank23)}
  ].filter(x=>x.score!==null&&x.score>0);
}
function admissionBandAnalysis(s,g){
  const target=currentAdmissionBandScore();
  const history=groupAdmissionHistory(g);
  if(target===null){
    return {band:'待定',className:'pending',reachable:0,total:history.length,target:null,partial:history.length<3,title:'未设置当前学生分数或目标分数，暂不能判断冲稳保。'};
  }
  if(!history.length){
    return {band:'待定',className:'pending',reachable:0,total:0,target,partial:true,title:`当前分 ${fmtNum(target)}；该专业组近三年没有可用专业组最低分，暂不能判断。`};
  }
  const reachable=history.filter(x=>target>=x.score).length;
  let band='稳';
  if(reachable===0)band='冲';
  else if(history.length>=3&&reachable===history.length)band='保';
  const className={冲:'rush',稳:'stable',保:'safe',待定:'pending'}[band]||'pending';
  const partial=history.length<3;
  const detail=history.map(x=>`${x.year} ${fmtNum(x.score)}分${target>=x.score?'够到':'未到'}`).join('；');
  const rule=band==='冲'?'三年都没有够到':band==='保'?'三年全部够到':'三年至少一年够到';
  const partialText=partial?'；历史不足三年，仅按已有年份暂判':'';
  return {band,className,reachable,total:history.length,target,partial,title:`当前分 ${fmtNum(target)}；${detail}；判定：${rule}${partialText}。`};
}
const DEFAULT_ADMISSION_RISK_CONFIG={
  chongMax:40,
  wenMax:80,
  descriptions:{chong:'较高风险',wen:'中等风险',bao:'较低风险',pending:'缺少概率'}
};
const RISK_LEVEL_META={
  chong:{riskText:'冲',className:'rush',color:'red'},
  wen:{riskText:'稳',className:'stable',color:'yellow'},
  bao:{riskText:'保',className:'safe',color:'green'},
  pending:{riskText:'待定',className:'pending',color:'gray'}
};
function admissionRiskConfig(){
  const input=window.ADMISSION_RISK_CONFIG||{};
  return {...DEFAULT_ADMISSION_RISK_CONFIG,...input,descriptions:{...DEFAULT_ADMISSION_RISK_CONFIG.descriptions,...(input.descriptions||{})}};
}
...
function explicitGroupProbability(g){
  return clampProbability(g?.admissionProbability??g?.probability??g?.riskProbability??g?.admissionProb??g?.chance);
}
function groupRiskAnchorScore(s,g){
  const direct=[g?.score25,g?.score26Eq,g?.avgScore3].map(num).find(v=>v!==null&&v>0);
  if(direct!==undefined)return direct;
  const predicted=groupScoreEstimate(s,g);
  if(predicted&&num(predicted.score)!==null&&num(predicted.score)>0)return num(predicted.score);
  const majorScores=(g?.majors||[]).flatMap(m=>[num(m.score25),num(m.avgScore3)]).filter(v=>v!==null&&v>0).sort((a,b)=>a-b);
  return majorScores.length?majorScores[0]:null;
}
function estimateAdmissionProbabilityFromHistory(s,g,target=currentAdmissionBandScore()){
  if(target===null)return null;
  const history=groupAdmissionHistory(g);
  const anchor=history.length?null:groupRiskAnchorScore(s,g);
  if(!history.length&&anchor===null)return null;
  const margins=history.length?history.map(x=>target-x.score):[target-anchor];
  const avg=margins.reduce((a,b)=>a+b,0)/margins.length;
  const reachable=margins.filter(x=>x>=0).length/margins.length;
  const logistic=100/(1+Math.exp(-avg/6));
  return Math.max(1,Math.min(99,Math.round(logistic*.65+reachable*100*.35)));
}
function riskLevelFromProbability(probability){
  const p=clampProbability(probability);
  if(p===null)return 'pending';
  const cfg=admissionRiskConfig();
  if(p<cfg.chongMax)return 'chong';
  if(p<cfg.wenMax)return 'wen';
  return 'bao';
}
function groupRiskState(s,g,key=keyGroup(s,g)){
  const explicit=explicitGroupProbability(g);
  const probability=explicit!==null?explicit:estimateAdmissionProbabilityFromHistory(s,g);
  const riskLevel=riskLevelFromProbability(probability);
  const meta=RISK_LEVEL_META[riskLevel]||RISK_LEVEL_META.pending;
  const cfg=admissionRiskConfig();
  const selectedOrder=volunteerKeys.indexOf(key);
  const selected=selectedOrder>=0;
  const history=admissionBandAnalysis(s,g);
  const probabilityText=probability===null?'—':`${probability}%`;
  const riskDesc=cfg.descriptions?.[riskLevel]||meta.riskText;
  const estimateNote=history.total?history.title:`当前分 ${fmtNum(currentAdmissionBandScore())}；缺少完整三年专业组最低分，已用 25分/26等效分/预测分/组内专业分兜底估算概率。`;
  const title=probability===null?`${riskDesc}；${history.title||'缺少录取概率和可用历史分。'}`:`录取概率 ${probabilityText}；${meta.riskText}｜${riskDesc}。阈值：冲 <${cfg.chongMax}%；稳 ${cfg.chongMax}%–${cfg.wenMax-1}%；保 ≥${cfg.wenMax}%。${estimateNote?` ${estimateNote}`:''}`;
  return {groupId:key,selected,selectedOrder:selected?selectedOrder+1:0,selectedText:selected?`已选 第${selectedOrder+1}志愿`:'未选',riskLevel,riskText:meta.riskText,probability,probabilityText,riskDesc,color:meta.color,className:meta.className,title};
}
function admissionBandBadgeHTML(s,g){
  const r=groupRiskState(s,g);
  return `<span class="tag admission-band admission-band-${r.className}" title="${esc(r.title)}">${esc(r.riskText)} ${esc(r.probabilityText)}</span>`;
}
function admissionBandRank(band){return {保:3,稳:2,冲:1,待定:0}[band]??0;}
function admissionBandCounts(){return {冲:0,稳:0,保:0,待定:0,total:0};}
function countAdmissionBand(summary,band){
  const key=summary.hasOwnProperty(band)?band:'待定';
  summary[key]+=1;
  summary.total+=1;
}
function admissionBandSummaryForGroups(s,groups){
  const summary=admissionBandCounts();
  (groups||[]).forEach(g=>countAdmissionBand(summary,groupRiskState(s,g).riskText));
  return summary;
}
function admissionBandSummaryForFiltered(){
  const summary=admissionBandCounts();
  state.filtered.forEach(s=>(s.visibleGroups||[]).forEach(g=>countAdmissionBand(summary,groupRiskState(s,g).riskText)));
  return summary;
}
function admissionBandSchoolSummaryForFiltered(){
  const summary=admissionBandCounts();
  state.filtered.forEach(s=>{
    let best='待定';
    (s.visibleGroups||[]).forEach(g=>{
      const band=groupRiskState(s,g).riskText;
      if(admissionBandRank(band)>admissionBandRank(best))best=band;
    });
    countAdmissionBand(summary,best);
  });
  return summary;
}
function admissionBandSummaryText(summary,unit=''){
  const pending=summary.待定?` 待定${summary.待定}${unit}`:'';
  return `冲${summary.冲}${unit} 稳${summary.稳}${unit} 保${summary.保}${unit}${pending}`;
}
function selectedStateBadgeHTML(s,g){
  const r=groupRiskState(s,g);
  return `<span class="tag selected-state-badge ${r.selected?'selected':'unselected'}" data-selected-state-key="${esc(r.groupId)}" title="${esc(r.selected?'已在右侧志愿表中':'尚未加入右侧志愿表')}">${esc(r.selectedText)}</span>`;
}
function groupRiskHeroContentHTML(s,g,key=keyGroup(s,g)){
  const r=groupRiskState(s,g,key);
  return `<div class="group-risk-hero-main"><b>${esc(r.probabilityText)}</b><strong>${esc(r.riskText)}</strong><span>${esc(r.riskDesc)}</span></div><em>${esc(r.selectedText)}</em>`;
}
function groupRiskHeroHTML(s,g,compact=false){
  const key=keyGroup(s,g);
  const r=groupRiskState(s,g,key);
  return `<div class="group-risk-hero risk-${esc(r.riskLevel)} ${compact?'compact':''} ${r.selected?'selected':'unselected'}" data-risk-hero-key="${esc(key)}" data-risk-hero-compact="${compact?'1':'0'}" title="${esc(r.title)}">${groupRiskHeroContentHTML(s,g,key)}</div>`;
}
function groupMatchesRisk(s,g){
  return !state.riskFilter||groupRiskState(s,g).riskText===state.riskFilter;
}
...
function batchGuideEligibleForBatch(def,s,g){
  ...
  return /提前|定向军士|定向培养军士|士官|军士生/.test(text);
}
...
function volunteerButtonHTML(s,g){
  const key=keyGroup(s,g);
  const risk=groupRiskState(s,g,key);
  ...
  const disabled=!selected&&(volunteerKeys.length>=VOLUNTEER_LIMIT||Boolean(subjectBlock));
  const label=selected?`已加入 第${risk.selectedOrder}志愿`:subjectBlock?'选科不符':disabled?'志愿表已满':'加入志愿表';
  const title=selected?'再次点击：从志愿表移出该专业组，并清空本组已选专业':subjectBlock?subjectBlock:allMedicalBlocked?'该组全部专业触发体检限报提醒；仍可加入，需核对招生章程':'点击：加入志愿表';
  const cls=`volunteer-add-btn risk-${risk.riskLevel} ${selected?'selected':''}`.trim();
...
function createLayout(){
  ...
  <div class="app-shell">
    ...
    <button id="volunteerPanelBtn" class="volunteer-toggle" type="button"><span>志愿表 0</span><small>冲0｜稳0｜保0</small><b>保存</b></button>
...
function fillBatchSelect(sel,batches){
  ...
  const specialtyCounts=specialtyBatchOptionCounts();
  const specialtyOptions=SPECIALTY_BATCH_FILTERS.map(def=>{
    const count=specialtyCounts.get(def.value)||0;
    return `<option value="${esc(def.value)}">${esc(def.label)}${count?`（${count}组）`:''}</option>`;
  }).join('');
  const rawBatches=batches.filter(v=>String(v)!=='专科批');
  const batchOptions=rawBatches.map(v=>`<option value="${esc(v)}">${esc(v)}</option>`).join('');
  ...
  const guideOptions=guideDefs.map(def=>{
  ...
  el.innerHTML=first+(specialtyOptions?`<optgroup label="专科批次">${specialtyOptions}</optgroup>`:'')+(guideOptions?`<optgroup label="提前批细分">${guideOptions}</optgroup>`:'')+(batchOptions?`<optgroup label="源表批次">${batchOptions}</optgroup>`:'');
}
...
function schoolTierText(s,d={}){
  return `${s?.level||''} ${d.schoolTags||''} ${d.firstClass||''} ${d.schoolLevel||''} ${d.schoolType||''} ${d.publicPrivate||''} ${d.administration||''}`;
}
function hasSchoolTierToken(text,token){
  return new RegExp(`(^|[^0-9A-Za-z])${token}([^0-9A-Za-z]|$)`).test(String(text||''));
}
function schoolTierFlags(s,d={}){
  const text=schoolTierText(s,d);
  const is985=hasSchoolTierToken(text,'985')||/985工程/.test(text);
  const has211=hasSchoolTierToken(text,'211')||/211工程/.test(text);
  const is211=has211&&!is985;
  ...
  return {text,is985,is211,isDoubleFirst,hasBaoyan,isPublic,isPrivate};
}
function schoolCoreTierLabels(s,d={}){
  const f=schoolTierFlags(s,d);
  const labels=[];
  if(f.is985)labels.push('985');
  else if(f.is211)labels.push('211');
  if(f.isDoubleFirst)labels.push('双一流');
  if(/国重点|国家重点/.test(f.text))labels.push('国重点');
  if(f.hasBaoyan)labels.push('保研资格');
  return [...new Set(labels)];
}
function shouldKeepRawSchoolLevel(v){
  const t=String(v??'').trim();
  if(!isValidFacetValue(t))return false;
  if((/985|211|双一流|国重点|国家重点|保研/.test(t))&&/[\/、,，\s]+/.test(t))return false;
  return true;
}
function schoolLevelDisplay(s,d={}){
  return schoolCoreTierLabels(s,d).join('/')||d.schoolTags||d.firstClass||d.schoolLevel||s?.level||'';
}
...
function schoolFacetValues(s){
  ...
  if(shouldKeepRawSchoolLevel(s.level))add(s.level);
  const tier=schoolTierFlags(s,d);
  const text=tier.text;
  schoolCoreTierLabels(s,d).forEach(add);
  if(tier.hasBaoyan&&!tier.is985&&!tier.is211&&!tier.isDoubleFirst)add('保研双非');
  if(tier.isPublic)add('公办');
  if(tier.isPrivate)add('民办');
  if(tier.isPublic&&!tier.is985&&!tier.is211&&!tier.isDoubleFirst&&!tier.hasBaoyan)add('普通公办');
...
function clampVolunteerTogglePosition(left,top,btn){
  const margin=10;
  const width=btn?.offsetWidth||78;
  const height=btn?.offsetHeight||132;
  return {
    left:Math.max(margin,Math.min(window.innerWidth-width-margin,left)),
    top:Math.max(margin,Math.min(window.innerHeight-height-margin,top))
  };
}
function readVolunteerTogglePosition(){
  try{
    const pos=JSON.parse(localStorage.getItem(VOLUNTEER_TOGGLE_POSITION_KEY)||'null');
...
function setVolunteerTogglePosition(btn,left,top){
  btn.style.setProperty('left',`${left}px`,'important');
  btn.style.setProperty('top',`${top}px`,'important');
  btn.style.setProperty('right','auto','important');
  btn.style.setProperty('transform','none','important');
}
function applyVolunteerTogglePosition(){
  const btn=$('#volunteerPanelBtn');
  const pos=btn?readVolunteerTogglePosition():null;
  if(!btn||!pos)return;
  const fixed=clampVolunteerTogglePosition(pos.left,pos.top,btn);
  setVolunteerTogglePosition(btn,fixed.left,fixed.top);
}
function bindVolunteerToggleDrag(){
  const btn=$('#volunteerPanelBtn');
  if(!btn||btn.dataset.dragBound)return;
  btn.dataset.dragBound='1';
  btn.title='可拖动调整位置；点击打开志愿表';
  applyVolunteerTogglePosition();
  let drag=null;
  ...
  btn.addEventListener('pointermove',e=>{
    ...
    e.preventDefault();
    const next=clampVolunteerTogglePosition(drag.left+dx,drag.top+dy,btn);
    setVolunteerTogglePosition(btn,next.left,next.top);
  });
  const finish=e=>{
    if(!drag||drag.id!==e.pointerId)return;
    ...
    if(!wasMoved)return;
    const rect=btn.getBoundingClientRect();
    localStorage.setItem(VOLUNTEER_TOGGLE_POSITION_KEY,JSON.stringify({left:rect.left,top:rect.top}));
    btn.dataset.dragSuppressUntil=String(Date.now()+300);
  };
  btn.addEventListener('pointerup',finish);
  btn.addEventListener('pointercancel',finish);
  window.addEventListener('resize',applyVolunteerTogglePosition);
}
function bindEvents(){
  ...
  const volunteerPanelBtn=$('#volunteerPanelBtn');
  volunteerPanelBtn?.addEventListener('click',()=>{if(Number(volunteerPanelBtn.dataset.dragSuppressUntil||0)>Date.now())return;renderVolunteerPanel();openPanel('volunteerPanel')});
  bindVolunteerToggleDrag();
  ...
  $('#volunteerViewSelect')?.addEventListener('change',e=>{volunteerViewMode=e.target.value||'order';renderVolunteerPanel({skipCapture:true});});
...
function groupSpecialTypeValues(s,g){
  ...
  add(specialtyBatchLabel(s,g));
  if(isDirectedSergeantGroup(s,g))add('定向军士');
  if(/定向军士男/.test(text))add('定向军士男');
  if(/定向军士女/.test(text))add('定向军士女');
  groupSegmentedModeValues(g).forEach(v=>add(v));
  ...
  if(groupTrainingModeValues(s,g).includes('联合培养'))add('联合培养');
  ...
  if(/定向医学生|免费医学生|免费医学定向|农村订单定向医学生|订单定向医学生|定向医学/.test(text)||groupTrainingModeValues(s,g).includes('定向医学'))add('定向医学生');
...
function schoolMatchesRegionFacet(s){
  const hasProvince=state.selectedProvinces.size>0;
  const hasCity=state.selectedCities.size>0;
  if(!hasProvince&&!hasCity)return true;
  const province=String(s?.province||'').trim();
  const city=String(s?.city||'').trim();
  if(province==='江苏'&&hasCity)return state.selectedCities.has(city);
  if(hasProvince)return state.selectedProvinces.has(province);
  return false;
}
...
function schoolTierSearchToken(q){
  const nq=normalize(q);
  if(/^985(大学|高校|院校)?$/.test(nq)||nq==='九八五')return '985';
  if(/^211(大学|高校|院校)?$/.test(nq)||['二一一','二幺一','二幺幺'].includes(nq))return '211';
  if(['双一流','一流大学','一流学科'].includes(nq))return '双一流';
  if(['保研','保研资格','推免资格'].includes(nq))return '保研资格';
  if(nq==='国家重点')return '国重点';
  if(['保研双非','普通公办','公办','民办','国重点'].includes(nq))return nq;
  return '';
}
function schoolMatchesTierSearch(s,token){
  if(!token)return false;
  return [...schoolFacetValues(s)].some(v=>normalize(v)===token);
}
function groupMatchesSearch(s,g,q){
  if(!q)return true;
  const nq=normalize(q);
  const tierToken=schoolTierSearchToken(q);
  if(tierToken)return schoolMatchesTierSearch(s,tierToken);
  const schoolFacetText=[...schoolFacetValues(s)].join('');
  if(normalize(s.name+s.province+s.city+s.subject+s.batch+schoolFacetText).includes(nq))return true;
  if(normalize(g.groupName+groupDisplayName(s,g)+g.requirement+(g.tags||[]).join('')+(g.majorClasses||[]).join('')+g.majorSummary).includes(nq))return true;
  return g.majors.some(m=>normalize(m.name+m.majorClass+m.discipline+m.code).includes(nq));
}
...
function applyFilters(){
  ...
  DB.forEach(s=>{
    if(state.batch&&!guideFilter&&!isSpecialtyBatchFilterValue(state.batch)&&s.batch!==state.batch)return;
    if(state.subject&&s.subject!==state.subject)return;
    if(!schoolMatchesRegionFacet(s))return;
    if(!schoolMatchesLevelFacet(s))return;
    const groups=s.groups.filter(g=>{if(!groupMatchesSpecialtyBatch(s,g))return false; if(!groupMatchesBatchGuideFilter(s,g))return false; if(!groupMatchesSpecialSelection(s,g))return false; if(!groupMatchesRequirement(g))return false; if(!groupMatchesScore(s,g))return false; if(!groupMatchesClass(g))return false; if(!groupMatchesRisk(s,g))return false; if(!groupMatchesSearch(s,g,q))return false; return true;});
...
function renderSidebarControls(total,visible){
  ...
  const start=sidebarBrowseMode==='paged'?sidebarPage*SIDEBAR_PAGE_SIZE+1:1;
...
function schoolSidebarRiskHTML(s){
  ...
  const states=groups.map(g=>groupRiskState(s,g)).filter(Boolean);
  const best=states.filter(x=>x.probability!==null).sort((a,b)=>(b.probability??-1)-(a.probability??-1))[0]||states[0];
  const selected=states.filter(x=>x.selected);
  const summary=admissionBandSummaryForGroups(s,groups);
  const selectedText=selected.length?`已选 ${selected.length}组`:'未选';
  return `<div class="sidebar-risk-line risk-${esc(best.riskLevel)}" title="${esc(best.title)}"><b>${esc(best.probabilityText)}</b><span>${esc(best.riskText)}｜${esc(best.riskDesc)}</span><em>${esc(selectedText)}</em><small>冲${summary.冲}｜稳${summary.稳}｜保${summary.保}</small></div>`;
}
...
function renderSidebar(){
  ...
  const groupBands=admissionBandSummaryForFiltered();
  const schoolBands=admissionBandSchoolSummaryForFiltered();
  ...
  $('#schoolList').innerHTML=visibleSchools.map(s=>`<div class="school-item ${s.id===state.activeSchoolId?'active':''} ${admissionPriorityInfo(s)?'priority-admission-school':''}" data-school-id="${s.id}" data-note-scope="schools" data-note-key="${esc(keySchool(s))}"><div class="name">${esc(s.name)}${admissionPriorityBadge(s,true)}${noteBadge('schools',keySchool(s))}</div><div class="meta"><span>${esc(displayRegionLabel(s))}</span><span>${esc(s.subject)}</span><span>${esc(schoolSpecialtyBatchSummary(s,s.visibleGroups))}</span><span>${s.visibleGroups.length}组</span></div>${schoolSidebarRiskHTML(s)}<div class="score-pill">加权均分 ${fmtNum(s.weightedScore)}</div></div>`).join('');
...
function renderGroupMode(){
  ...
  const summary=admissionBandSummaryForFiltered();
  $('#main').innerHTML=`<div class="school-header"><div class="school-title"><h2>专业组视图</h2><span class="badge green">当前显示 ${shown.length} / ${groups.length} 组</span></div><div class="badges"><span class="badge">按院校加权均分排序</span><span class="badge">分数筛选含新增组预测分</span><span class="badge band-summary">${esc(admissionBandSummaryText(summary,'组'))}</span></div></div>`+shown.map(([s,g])=>groupSectionHTML(s,g)).join('')+(groups.length>limit?`<div class="empty">为保证浏览器性能，当前展示前 ${limit} 个专业组；可以继续缩小筛选条件。</div>`:'');
  bindDynamic();
}
...
function majorRowHTML(s,g,m){
  ...
  const disabled=(subjectBlock||qualificationBlocked)?' disabled':'';
  const physicalTitle=physical.blocked?`体检受限：${physical.reason}`:'';
  const qualificationTitle=qualification?.summary||'';
  const segmentedBadges=segmentedModeBadgesHTML(majorSegmentedModeValues(m,g),'major');
...
function schoolInfoPairs(s,d={}){const priority=admissionPriorityInfo(s); return [
  ['院校名称',d.school||s?.name],['录取规则风险',priority?`${priority.label}｜${priority.rules[0]||''}`:''],['所在省份',d.province||s?.province],['所在城市',d.city||s?.city],['城市层级',d.cityLevelTag],['院校代码',d.schoolCode],['院校标签',schoolLevelDisplay(s,d)],['院校层次',d.schoolLevel||s?.level],['隶属单位',d.administration],['院校类型',d.schoolType],['公私性质',d.publicPrivate],['本科/专科',d.bachelorSpecialty],['保研率',d.baoyanRate],['学校软科排名',d.schoolRank],['转专业政策',d.transferPolicy],['硕士点数量',d.schoolMasterCount],['硕士点',d.masterPrograms],['博士点数量',d.schoolDoctorCount],['博士点',d.doctorPrograms],['录取规则',d.admissionRule],['招生章程',d.recruitChapter]
];}
...
function majorRiskDetailPairs(s,g,m){
  ...
  return [
    ['专业热度梯队',tier.label],
    ['梯队分类',tier.bucket],
    ['报考资格匹配',qualification?.issues?.length?qualification.summary:'未识别到额外资格限制'],
...
function mergedMajorDetail(key,ctx){
  DETAILS=window.MAJOR_DETAILS||window.DETAILS||DETAILS||{};
  return enrichMajorDetail(Object.assign({}, fallbackMajorDetail(ctx), DETAILS[key]||{}),ctx);
}
...
function showDetail(key,schoolKey,groupKey){
  ...
  $('#modal').innerHTML=`<h3>${esc(heading)}</h3><div class="modal-body"><div class="info-subtitle">已同步 Excel 行级字段：专业代码、专业名称、专业备注、专业类、学制、学费、计划与录取数据。<br>身份键：${esc(d.identityKey||key)}｜数据源行：${esc(d.sourceExcelRow||'')}</div>${riskSection}${infoSectionHTML('院校基础信息',schoolInfoPairs(null,d),'school-info-section')}${parentSchoolInfoHTML(d.school||ctx?.s?.name,{city:d.city||ctx?.s?.city,address:d.address,level:ctx?.s?.level,schoolLevel:d.schoolLevel,publicPrivate:d.publicPrivate})}${infoSectionHTML('专业组基础信息',groupInfoPairs(d),'group-info-section')}${infoSectionHTML('专业基础信息',majorBasePairs(d),'major-info-section major-plan-info-section')}${infoSectionHTML('招生录取数据',admissionInfoPairs(d),'admission-info-section')}<div class="badges">${schoolNote?`<span class="note-badge">学校备注</span>`:''}${groupNote?`<span class="note-badge">专业组备注</span>`:''}${majorNote?`<span class="note-badge">专业备注</span>`:''}</div>${noteBlock('学校备注',schoolNote)}${noteBlock('专业组备注',groupNote)}${noteBlock('专业备注',majorNote)}<div class="modal-actions"><button onclick="document.getElementById('modalMask').classList.remove('open')">关闭</button></div></div>`;
  openModal();
}
...
function uniqueValidMajorKeys(keys,g){
  ...
  (Array.isArray(keys)?keys:[]).forEach(k=>{
    if(valid.has(k)&&!out.includes(k)&&out.length<MAX_MAJOR_PER_GROUP){
      ...
      if(m&&!qualificationRiskBlocksSelection(qualificationRiskForMajor(null,g,m)))out.push(k);
    }
  });
  return out;
}
...
function setMajorSelection(key,majorKey,checked){
  ...
  if(checked){
    ...
    if(physical&&physical.blocked&&!confirm(`体检风险提醒：该专业触发限报提示。\n${physical.reason}\n\n不同院校执行口径可能不同，请核对招生章程。仍然选择该专业吗？`))return false;
...
function updateVolunteerUI(){
  ...
  if(btn){
    ...
    btn.innerHTML=`<span>志愿表 ${count}</span><small>${esc(volunteerRiskInline(counts))}</small><b>保存</b>`;
    btn.title=`已选 ${count}/${VOLUNTEER_LIMIT} 个专业组；${volunteerRiskInline(counts)}；可拖动调整位置`;
    applyVolunteerTogglePosition();
  }
  updateStudentProfileMenu();
  renderVolunteerStructureBar();
  const countEl=$('#volunteerPanelCount');
  if(countEl)countEl.textContent=`已选 ${count} / ${VOLUNTEER_LIMIT} 个专业组｜${volunteerRiskInline(volunteerStrategyCounts(volunteerKeys))}`;
  $$('[data-volunteer-key]').forEach(btn=>{
    ...
    const allMedicalBlocked=Boolean(med&&med.all);
    const subjectBlock=rec?subjectRequirementBlockReason(rec.s,rec.g):'';
    const risk=rec?groupRiskState(rec.s,rec.g,btn.dataset.volunteerKey):null;
    const disabled=!selected&&(volunteerKeys.length>=VOLUNTEER_LIMIT||Boolean(subjectBlock));
    btn.textContent=selected?`已加入 第${risk?.selectedOrder||volunteerKeys.indexOf(btn.dataset.volunteerKey)+1}志愿`:subjectBlock?'选科不符':disabled?'志愿表已满':'加入志愿表';
    btn.title=selected?'再次点击：从志愿表移出该专业组，并清空本组已选专业':subjectBlock?subjectBlock:allMedicalBlocked?'该组全部专业触发体检限报提醒；仍可加入，需核对招生章程':'点击：加入志愿表';
    btn.classList.toggle('selected',selected);
    ['risk-chong','risk-wen','risk-bao','risk-pending'].forEach(c=>btn.classList.remove(c));
    if(risk)btn.classList.add(`risk-${risk.riskLevel}`);
    btn.disabled=disabled;
  });
  $$('[data-selected-state-key]').forEach(badge=>{
    const rec=getGroupRecord(badge.dataset.selectedStateKey);
    if(!rec)return;
    const risk=groupRiskState(rec.s,rec.g,badge.dataset.selectedStateKey);
    badge.textContent=risk.selectedText;
    badge.title=risk.selected?'已在右侧志愿表中':'尚未加入右侧志愿表';
    badge.classList.toggle('selected',risk.selected);
    badge.classList.toggle('unselected',!risk.selected);
  });
  $$('[data-risk-hero-key]').forEach(hero=>{
    const rec=getGroupRecord(hero.dataset.riskHeroKey);
    if(!rec)return;
    const risk=groupRiskState(rec.s,rec.g,hero.dataset.riskHeroKey);
    hero.innerHTML=groupRiskHeroContentHTML(rec.s,rec.g,hero.dataset.riskHeroKey);
    hero.title=risk.title;
    hero.classList.toggle('selected',risk.selected);
    hero.classList.toggle('unselected',!risk.selected);
    ['risk-chong','risk-wen','risk-bao','risk-pending'].forEach(c=>hero.classList.remove(c));
    hero.classList.add(`risk-${risk.riskLevel}`);
  });
}
function addVolunteerKey(key){
  ...
  if(!volunteerKeys.includes(key)&&med&&med.all&&!confirm('体检风险提醒：当前体检代码下，该专业组全部专业都触发限报提示。\n不同学校可能执行口径不同，请核对招生章程。仍然加入志愿表吗？'))return;
  if(!volunteerKeys.includes(key)){
...
const VOLUNTEER_FILL_STRATEGIES={
  '1':{label:'均衡型',ratios:{冲:.30,稳:.45,保:.25}},
  '2':{label:'保守型',ratios:{冲:.20,稳:.40,保:.40}},
  '3':{label:'进取型',ratios:{冲:.40,稳:.40,保:.20}}
};
function chooseVolunteerFillStrategy(){
  const input=prompt('选择一键填报策略：\n1 均衡型：冲30%｜稳45%｜保25%\n2 保守型：冲20%｜稳40%｜保40%\n3 进取型：冲40%｜稳40%｜保20%','1');
  if(input===null)return null;
  return VOLUNTEER_FILL_STRATEGIES[String(input).trim()]||VOLUNTEER_FILL_STRATEGIES['1'];
}
function fillVolunteerFromCurrentFilters(){
  const strategy=chooseVolunteerFillStrategy();
  if(!strategy)return;
  const buckets={冲:[],稳:[],保:[],待定:[]};
  for(const s of state.filtered){
    for(const g of s.visibleGroups){
      const key=keyGroup(s,g);
      const med=groupMedicalRestrictionSummary(g);
      const subjectBlock=subjectRequirementBlockReason(s,g);
      if(!volunteerKeys.includes(key)&&!subjectBlock){
        const risk=groupRiskState(s,g,key).riskText;
        (buckets[risk]||buckets.待定).push({key,medicalAll:Boolean(med&&med.all)});
      }
    }
  }
  const room=VOLUNTEER_LIMIT-volunteerKeys.length;
  if(room<=0){alert(`志愿表最多 ${VOLUNTEER_LIMIT} 个专业组，当前已满。`);return;}
  const plan={冲:Math.round(room*(strategy.ratios.冲||0)),稳:Math.round(room*(strategy.ratios.稳||0)),保:Math.round(room*(strategy.ratios.保||0))};
  let picked=[];
  ['冲','稳','保'].forEach(name=>{picked=picked.concat(buckets[name].splice(0,Math.max(0,Math.min(plan[name],room-picked.length))));});
  ['稳','保','冲','待定'].forEach(name=>{while(picked.length<room&&buckets[name]?.length)picked.push(buckets[name].shift());});
  picked.forEach(item=>{
    if(volunteerKeys.length<VOLUNTEER_LIMIT&&!volunteerKeys.includes(item.key)){
      volunteerKeys.push(item.key);
      ensureVolunteerSelection(item.key);
    }
  });
  if(picked.some(x=>x.medicalAll))alert('已加入的部分专业组全部专业触发体检风险提醒，请在专业表中逐项核对学校招生章程。');
  saveVolunteerKeys();
  saveVolunteerMajorKeys();
  saveVolunteerMeta();
  renderVolunteerPanel();
  updateVolunteerUI();
  render();
  alert(`已按${strategy.label}补入 ${picked.length} 个专业组。当前结构：${volunteerRiskInline(volunteerStrategyCounts(volunteerKeys))}`);
}
...
function volunteerSearchText(key,rec){
  ...
  const strategy=volunteerStrategyValue(key,rec);
  ...
  return `${s.name||''} ${s.province||''} ${s.subject||''} ${s.batch||''} ${g.groupName||''} ${g.groupCode||''} ${groupDisplayName(s,g)||''} ${g.requirement||''} ${(g.tags||[]).join(' ')} ${strategy} ${meta.note||''} ${selectedText} ${majorText}`;
}
function volunteerEntryMatches(key,rec){
  ...
  const strategy=volunteerStrategyValue(key,rec);
  if(['冲','稳','保','垫'].includes(mode))return strategy===mode;
  if(mode==='pending')return strategy==='待定';
...
function volunteerDisplayEntries(entries){
  if(volunteerViewMode!=='risk')return entries;
  const order={冲:0,稳:1,保:2,垫:3,待定:4};
  return [...entries].sort((a,b)=>(order[volunteerStrategyValue(a.key,a.rec)]??9)-(order[volunteerStrategyValue(b.key,b.rec)]??9)||a.index-b.index);
}
function volunteerRiskGroupHeaderHTML(item,prev){
  if(volunteerViewMode!=='risk')return '';
  const current=volunteerStrategyValue(item.key,item.rec);
  const before=prev?volunteerStrategyValue(prev.key,prev.rec):'';
  if(current===before)return '';
  const meta=VOLUNTEER_STRATEGY_META[current]||VOLUNTEER_STRATEGY_META['待定'];
  return `<div class="volunteer-risk-group-header strategy-${meta.className}">${esc(current)}类专业组</div>`;
}
...
function volunteerStrategyValue(key,rec=getGroupRecord(key)){
  const value=String(volunteerMeta[key]?.strategy||'').trim();
  if(VOLUNTEER_STRATEGY_META[value])return value;
  if(rec){
    const auto=groupRiskState(rec.s,rec.g,key).riskText;
    if(VOLUNTEER_STRATEGY_META[auto])return auto;
  }
  return '待定';
}
function volunteerStrategyPillHTML(key){
  const rec=getGroupRecord(key);
  const value=volunteerStrategyValue(key,rec);
  const meta=VOLUNTEER_STRATEGY_META[value]||VOLUNTEER_STRATEGY_META['待定'];
  const manual=Boolean(String(volunteerMeta[key]?.strategy||'').trim());
  const risk=rec?groupRiskState(rec.s,rec.g,key):null;
  const title=manual?`人工定位：${meta.label}`:(risk?.title||`自动定位：${meta.label}`);
  return `<span class="volunteer-strategy-pill strategy-${meta.className}" title="${esc(title)}">${esc(meta.label)}${risk?.probabilityText&&risk.probabilityText!=='—'?` ${esc(risk.probabilityText)}`:''}</span>`;
}
...
function volunteerRiskInline(counts=volunteerStrategyCounts(volunteerKeys)){
  return `冲 ${counts.冲||0}｜稳 ${counts.稳||0}｜保 ${counts.保||0}`;
}
function renderVolunteerStructureBar(){
  const bar=$('#volunteerStructureBar');
  if(!bar)return;
  const selectedCounts=volunteerStrategyCounts(volunteerKeys);
  const visibleCounts=admissionBandSummaryForFiltered();
  const selectedTotal=volunteerKeys.length;
  const totalForTrack=Math.max(selectedTotal,1);
  const seg=name=>{
    const count=selectedCounts[name]||0;
    const meta=VOLUNTEER_STRATEGY_META[name]||VOLUNTEER_STRATEGY_META['待定'];
    return `<span class="volunteer-structure-seg strategy-${meta.className}" style="--w:${count/totalForTrack*100}%"></span>`;
  };
  bar.hidden=false;
  bar.innerHTML=`<div class="volunteer-structure-main"><b>已选 ${selectedTotal} / ${VOLUNTEER_LIMIT}</b><span>${esc(volunteerRiskInline(selectedCounts))}</span><em>当前列表：${esc(admissionBandSummaryText(visibleCounts,'组'))}</em></div><div class="volunteer-structure-track">${['冲','稳','保','待定'].map(seg).join('')}</div><div class="volunteer-structure-actions"><button type="button" data-risk-filter="" class="${state.riskFilter?'':'active'}">全部</button>${['冲','稳','保'].map(name=>`<button type="button" data-risk-filter="${name}" class="${state.riskFilter===name?'active':''}">${name} ${visibleCounts[name]||0}</button>`).join('')}</div>`;
  $$('[data-risk-filter]').forEach(btn=>btn.addEventListener('click',()=>{
...
function renderVolunteerPanel(options={}){
  ...
  const displayEntries=volunteerDisplayEntries(visible);
  list.innerHTML=batchGuideVolunteerSummaryHTML(visible)+displayEntries.map((entry,i)=>`${volunteerRiskGroupHeaderHTML(entry,displayEntries[i-1])}${volunteerRowHTML(entry.key,entry.index)}`).join('');
...
function volunteerRowHTML(key,index){
  ...
  const risk=groupRiskState(s,g,key);
  const strategyMeta=VOLUNTEER_STRATEGY_META[volunteerStrategyValue(key,{s,g})]||VOLUNTEER_STRATEGY_META['待定'];
  const autoOptionLabel=risk.riskText==='待定'?'自动待定':`自动${risk.riskText} ${risk.probabilityText}`;
  ...
  const majorPicker=`<details class="major-picker volunteer-edit-drawer"${detailsOpen}><summary>专业池 ${majors.length} 个｜已选 ${selectedOrder.length} / ${MAX_MAJOR_PER_GROUP}</summary><div class="major-picker-actions compact"><select data-major-pool-filter="${esc(key)}"><option value="all" ${poolFilter==='all'?'selected':''}>全部专业</option><option value="selected" ${poolFilter==='selected'?'selected':''}>只看已选</option><option value="unselected" ${poolFilter==='unselected'?'selected':''}>只看未选</option></select><button data-major-preset="${esc(key)}" data-preset="none">清空专业</button></div><div class="major-picker-grid volunteer-major-grid compact-grid">${majors.map(m=>{const order=selectedOrder.indexOf(m.key);const isSelected=order>=0;const hidden=(poolFilter==='selected'&&!isSelected)||(poolFilter==='unselected'&&isSelected);const riskMeta=majorRiskMeta(s,g,m);const physical=medicalRestrictionForMajor(m);const qualification=qualificationRiskForMajor(s,g,m);const blocked=Boolean(subjectBlock);const qualificationBlocked=qualificationRiskBlocksSelection(qualification);const qualificationTitle=qualification?.summary||'';const blockTitle=subjectBlock||qualificationTitle||physical.reason;return `<label class="major-check ${riskMeta.risk?'risk':''} ${riskMeta.tone?`risk-tone-${riskMeta.tone}`:''} ${physical.blocked?'physical-blocked':''} ${subjectBlock?'subject-blocked':''} ${qualificationBlocked?'qualification-blocked':''} ${isSelected?'selected':'unselected'}" title="${esc((blocked||qualificationBlocked)?blockTitle:(physical.reason||qualificationTitle||riskMeta.reason||''))}" data-major-pool-state="${isSelected?'selected':'unselected'}" ${hidden?'style="display:none"':''}><input type="checkbox" data-major-check="${esc(key)}" value="${esc(m.key)}" ${isSelected?'checked':''} ${(blocked||qualificationBlocked)?'disabled':''}>${isSelected?`<span class="major-order-badge">${order+1}</span>`:'<span class="major-order-placeholder"></span>'}<b><span class="major-name-text">${esc(m.name)}</span>${majorMetaBadgesHTML(m,'三')}</b>${medicalRestrictionLabelHTML(physical)}${subjectBlock?'<span class="risk-label">选科不符</span>':''}${qualificationRiskLabelHTML(qualification)}${majorRiskLabelHTML(riskMeta)}<small>${esc(m.majorClass||'其他')}｜${fmt(m.plan26)}人｜${fmtNum(m.score25)}分｜位次 ${fmtNum(m.rank25)}</small></label>`;}).join('')}</div></details>`;
  return `<article class="volunteer-item volunteer-table-row strategy-${strategyMeta.className}" data-volunteer-item="${esc(key)}">
    ...
    <div class="volunteer-group-col"><div class="volunteer-group-title"><b>${esc(groupShortTitle(s,g))}</b><span class="risk-inline strategy-${strategyMeta.className}" title="${esc(risk.title)}">${esc(risk.riskText)} ${esc(risk.probabilityText)}</span>${batchGuideBadgeHTML(s,g,true)}</div><p>再选 ${esc(g.requirement||'—')}</p><p class="volunteer-group-alias">${esc(groupAlias)}</p>${guideLine}</div>
...
function bindVolunteerPanelControls(){
  ...
  $$('[data-volunteer-locate]').forEach(btn=>btn.addEventListener('click',()=>locateVolunteerGroup(btn.dataset.volunteerLocate)));
...
function locateVolunteerGroup(key){
  const rec=getGroupRecord(key);
  if(!rec)return;
  state.activeSchoolId=rec.s.id;
  state.mode='schools';
  closePanel('volunteerPanel');
  render();
  setTimeout(()=>{
    const el=document.getElementById(rec.g.id);
    if(!el)return;
...
function exportVolunteerXlsx(){
  ...
  if(!confirmVolunteerWarnings('导出'))return;
  const now=new Date();
  ...
  for(let i=0;i<volunteerKeys.length;i++){
    ...
    const strategy=volunteerStrategyValue(key,{s,g});
    items.forEach((item,idx)=>{
      ...
      if(idx===0){
        rows.push([
          ...
          {value:strategy,style:'seq',mergeDown:merge},
...
function showAccessRequestModal(){
  $('#modal').innerHTML='<h3>申请开通</h3><div class="modal-body">'+
    ...
    '</div>';
  openModal();
  $('#copyAccessRequest')?.addEventListener('click',async function(){
...
function showModuleChoiceModal(){
  ...
  $('#goComprehensiveModule')?.addEventListener('click',()=>{window.location.href='../comprehensive/index.html';});
}
...
function studentSummary(s){
  ...
  return stageLabel(s.stage)+'｜'+studentSubjectSummary(s)+'｜'+(studentScore(s)||'—')+'分｜位次 '+(studentRank(s)||'—')+(medical.length?'｜体检 '+medical.join('/'):'')+((s.target_cities||[]).length?'｜城市 '+s.target_cities.join('、'):'');
}
...
function groupPayloadForSave(key,index){
  ...
  const {s,g}=rec;
  const meta=volunteerMeta[key]||{};
  return {form_id:null,owner_id:auth.user.id,position:index+1,group_key:key,school_name:s.name,school_code:s.schoolCode||null,province:s.province||null,city:s.city||null,batch:s.batch||null,subject:s.subject||null,group_name:g.groupName,group_code:g.groupCode||null,group_alias:groupDisplayName(s,g)||null,requirement:g.requirement||null,plan26:dbInteger(g.plan26),plan25:dbInteger(g.plan25),score25:dbNumber(g.score25),rank25:dbInteger(g.rank25),avg_score3:dbNumber(g.avgScore3),avg_rank3:dbInteger(g.avgRank3),strategy:volunteerStrategyValue(key,rec),obey_adjustment:true,note:meta.note||null,source_payload:{group:g,batchGuide:batchGuideSavePayload(s,g)}};
}
...
function volunteerRiskWarnings(){
  const total=volunteerKeys.length;
  if(!total)return [];
  const counts=volunteerStrategyCounts(volunteerKeys);
  const warnings=[];
  const minBao=Math.max(3,Math.round(total*.2));
  if(total>=10&&(counts.保||0)<minBao)warnings.push(`当前保类专业组 ${counts.保||0} 个，保底数量偏少，建议至少保留 ${minBao} 个左右。`);
  if(total>=10&&(counts.冲||0)/total>.45)warnings.push(`当前冲类专业组 ${counts.冲||0}/${total}，占比偏高，可能影响录取稳定性。`);
  return warnings;
}
function volunteerMedicalWarnings(){
  const warnings=[];
  volunteerKeys.forEach((key,index)=>{
    const rec=getGroupRecord(key);
    if(!rec)return;
    const selected=selectedMajorsForKey(key).filter(m=>medicalRestrictionForMajor(m).blocked);
    if(selected.length)warnings.push(`${index+1}. ${groupShortTitle(rec.s,rec.g)}：${selected.map(m=>m.name).join('、')} 触发体检风险提醒`);
  });
  return warnings;
}
function confirmVolunteerWarnings(action='保存'){
  const warnings=[...volunteerRiskWarnings(),...volunteerMedicalWarnings()];
  if(!warnings.length)return true;
  return confirm(`${action}前风险提醒：\n${warnings.slice(0,12).join('\n')}${warnings.length>12?'\n……':''}\n\n以上仅提示，不强制拦截。确认继续${action}吗？`);
}
