...
const GUIDE_DEFS=[
  {
    ...
    title:'提前批填报指南',
    scope:'提前批',
    summary:'用于核对提前批与普通批的关系、录取顺序、是否影响后续批次、报名体检面试政审等前置事项。',
    checks:['是否属于提前批','录取后是否锁定后续普通批','不录取是否影响普通批','是否需要提前报名','是否需要体检/面试/政审','是否有单独志愿栏或单独投档顺序'],
    trigger:/提前批|提前本科|提前专科|本科提前|专科提前|综合评价|强基|飞行技术|航海|军校|公安|司法警官|定向军士/
  },
  {
    id:'early-military',
    channel:'军校提前批',
    title:'军校报考指南',
    scope:'提前批 · 军校',
    summary:'军校志愿必须按军校提前批通道单独核对，不能和公安、司法、航海等提前批类型混在一个排序口径里。',
  ...
  },
  {
    ...
    title:'司法警校报考指南',
    scope:'提前批 · 司法警校',
    summary:'司法警校与公安不是同一通道，需单独核对司法行政警察类专业的政审、面试、体检、体测和入警政策。',
    checks:['提前批属性','司法行政警察类专业','政审','面试','体检','体测','入警政策','与普通法学专业区别'],
    trigger:/司法警官|中央司法警官|司法行政|司法警察|监狱学|刑事执行|行政执行|罪犯心理测量/
  },
  {
    id:'early-maritime',
    channel:'航海提前批',
    ...
    trigger:/航海技术|轮机工程|船舶电子电气|海事|航海类|轮机类|船舶驾驶/
  },
  {
    id:'early-sergeant',
    channel:'定向军士提前批',
...
const GUIDE_PRIORITY=['early-sergeant','early-military','early-police','early-judicial','early-maritime','early-common'];
...
function detect(ctx={}){
  ...
  const typed=GUIDE_DEFS.filter(d=>d.id!=='early-common'&&d.trigger.test(text));
  if(typed.length)return typed.sort((a,b)=>GUIDE_PRIORITY.indexOf(a.id)-GUIDE_PRIORITY.indexOf(b.id))[0];
  const common=GUIDE_DEFS.find(d=>d.id==='early-common');
  return common&&common.trigger.test(text)?common:null;
}
function allMatches(ctx={}){
  const text=contextText(ctx);
  const matches=GUIDE_DEFS.filter(d=>d.trigger.test(text));
  if(!matches.length)return [];
  const typed=matches.filter(d=>d.id!=='early-common');
  if(!typed.length)return matches;
  const sorted=typed.sort((a,b)=>GUIDE_PRIORITY.indexOf(a.id)-GUIDE_PRIORITY.indexOf(b.id));
  return sorted[0].id==='early-sergeant'?[sorted[0]]:sorted;
}
function urlFor(def){
...
function panelHTML(id){
  ...
  return `<section class="batch-guide-card">
    ...
    <h4>${esc(def.title)}</h4>
    <p>${esc(def.summary)}</p>
    <div class="batch-guide-warning"><b>填报逻辑：</b>${esc(def.channel)} 必须作为独立通道核对和排序；军校、公安、司法、航海、定向军士不能混用同一个提前批判断口径。</div>
    <h5>本地核对清单</h5>
    <ol>${checks}</ol>
...
function channelSummaryHTML(entries){
  ...
  const warn=counts.size>1?'<p class="batch-channel-warn">已出现多个提前批通道：请按军校、公安、司法、航海、定向军士分别核对，不要混成一个提前批排序口径。</p>':'';
