...
function renderList(warning=''){
  ...
  list.innerHTML=`${warning?`<div class="notice">${warning}</div>`:''}${rows.map(x=>{
    const attrs=x.file_url?`href="${esc(x.file_url)}" target="_blank" rel="noopener" data-view-id="${esc(x.id)}"`:'href="javascript:void(0)"';
    return `<a class="content-row" ${attrs}><span class="content-title">${esc(x.title||x.file_name||'未命名资料')}</span><time class="content-time">${esc(shortDate(x.created_at))}</time></a>`;
  }).join('')}`;
  document.querySelectorAll('[data-view-id]').forEach(row=>row.addEventListener('click',event=>{
    if(canInlinePreview(items.find(x=>String(x.id)===String(row.dataset.viewId))))event.preventDefault();
    showFile(row.dataset.viewId);
  }));
}
