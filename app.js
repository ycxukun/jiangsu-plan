(function(){
'use strict';
const VERSION='本科版｜V1.1.71 专业组单组展开版';
...
const activeGroupKeyBySchool=new Map();
...
function studentSubjectShortSummary(student){
  ...
  return `${type}${choices.length?choices.map(x=>map[x]||String(x).slice(0,1)).join(''):'未填'}`;
}
...
function admissionRiskConfig(){
  ...
  return {
    ...DEFAULT_ADMISSION_RISK_CONFIG,
    ...input,
    descriptions:{...DEFAULT_ADMISSION_RISK_CONFIG.descriptions,...(input.descriptions||{})}
  };
}
...
function estimateAdmissionProbabilityFromHistory(s,g,target=currentAdmissionBandScore()){
  ...
  const estimated=(logistic*.65+reachable*100*.35);
  return Math.max(1,Math.min(99,Math.round(estimated)));
}
...
function groupRiskState(s,g,key=keyGroup(s,g)){
  ...
  const estimateNote=history.total
    ? history.title
    : `当前分 ${fmtNum(currentAdmissionBandScore())}；缺少完整三年专业组最低分，已用 25分/26等效分/预测分/组内专业分兜底估算概率。`;
  const title=probability===null
    ? `${riskDesc}；${history.title||'缺少录取概率和可用历史分。'}`
    : `录取概率 ${probabilityText}；${meta.riskText}｜${riskDesc}。阈值：冲 <${cfg.chongMax}%；稳 ${cfg.chongMax}%–${cfg.wenMax-1}%；保 ≥${cfg.wenMax}%。${estimateNote?` ${estimateNote}`:''}`;
  return {
    groupId:key,
    selected,
    selectedOrder:selected?selectedOrder+1:0,
    selectedText:selected?`已选 第${selectedOrder+1}志愿`:'未选',
    riskLevel,
...
function createLayout(){
  ...
  <div class="app-shell">
    ...
    <div class="footer">
      <div>GitHub Pages 静态部署版 · 数据分片加载 · 公开页只读 / 管理员页可写备注</div>
      <div class="site-records" aria-label="网站备案信息">
        <a href="https://beian.miit.gov.cn/" target="_blank" rel="noopener noreferrer">苏ICP备2026045146号-1</a>
        <a href="https://beian.mps.gov.cn/#/query/webSearch?code=32090202001634" target="_blank" rel="noopener noreferrer"><img src="/beian-icon.png" alt="公安备案图标">苏公网安备32090202001634号</a>
      </div>
    </div>
  </div>`;
  document
...
function foreignCoopCountForGroup(g){return (g?.majors||[]).filter(hasForeignCoopDetail).length;}
function foreignCoopCountForGroups(groups){return (groups||[]).reduce((sum,g)=>sum+foreignCoopCountForGroup(g),0);}
function foreignCoopCountBadgeHTML(g){const count=foreignCoopCountForGroup(g);return count?`<span class="tag green foreign-coop-count">中外详情 ${count}</span>`:'';}
function foreignCoopSchoolAlertHTML(groups){
  const count=foreignCoopCountForGroups(groups);
  if(!count)return '';
  return `<div class="foreign-coop-alert"><b>已收录 ${count} 个中外合作专业详情</b><span>点击专业行里的“中外合作详情”，查看合作院校、学制、学费、是否出国、证书、保研/就业去向与评价。</span></div>`;
}
...
function groupCleanTagsHTML(s,g,planDiff,quality,compact=false){
  const pieces=[];
  const push=x=>{if(x&&String(x).trim())pieces.push(x);};
  push(groupQualityBadge(quality));
  push(selectedStateBadgeHTML(s,g));
  push(foreignCoopCountBadgeHTML(g));
  push(groupMedicalBadge(g));
  push(groupQualificationBadge(s,g));
  push(predictionBadgeHTML(s,g));
  if(planDiff!==0)push(planDeltaBadge(planDiff));
  const weighted=groupWeightedMajorScore(g);
  if(weighted&&num(weighted.score)!==null)push(`<span class="tag muted compact-stat">均分 ${fmtNum(weighted.score)}</span>`);
  if(!compact){
    const classNames=(g.majorClasses||[]).filter(Boolean);
    if(classNames.length)push(`<span class="tag muted compact-stat">${esc(classNames.slice(0,2).join(' / '))}${classNames.length>2?' 等':''}</span>`);
  }
  return pieces.join('');
}
function majorActionButtonsHTML(s,g,m,coopDetail,groupKey){
  ...
  return `<span class="major-inline-actions">${buttons.join('')}</span>`;
}
function foreignCoopHeroHTML(d={}){
  if(!d.foreignCoop)return '';
  const chips=[
    ['合作院校',d.foreignCoopPartner],
    ['学制/模式',d.foreignCoopDurationAbroad],
    ['是否出国',d.foreignCoopMustAbroad],
    ['学费',d.foreignCoopTuition],
    ['证书',d.foreignCoopCertificate],
    ['评价',d.foreignCoopEvaluation||d.foreignCoopReviewResult]
  ].filter(([,v])=>validInfoValue(v));
  return `<section class="foreign-coop-hero"><div class="foreign-coop-hero-title"><h4>中外合作项目详情</h4><span>已匹配到 985/211 中外合作补充库</span></div><div class="foreign-coop-hero-grid">${chips.map(([k,v])=>`<div><b>${esc(k)}</b><span>${esc(v)}</span></div>`).join('')}</div></section>`;
}
...
function activeGroupKeyForSchool(s,groups){
  const schoolKey=keySchool(s);
  const groupKeys=(groups||[]).map(g=>keyGroup(s,g));
  const saved=activeGroupKeyBySchool.get(schoolKey);
  const active=saved&&groupKeys.includes(saved)?saved:(groupKeys[0]||'');
  if(active)activeGroupKeyBySchool.set(schoolKey,active);
  else activeGroupKeyBySchool.delete(schoolKey);
  return active;
}
...
function groupCardHTML(s,g,activeGroupKey=''){
  ...
  const schoolKey=keySchool(s);
  const active=groupKey===activeGroupKey;
  const cardGroup=inlineMajorChangeGroup(s,g);
  const cutoff26=inlineMajorCutoff26HTML(cardGroup);
  return `<article class="group-card group-card-selector group-quality-${quality.tone} group-risk-${risk.riskLevel} ${risk.selected?'selected-volunteer-group':''} ${active?'is-active':''}" data-group-select="${esc(groupKey)}" data-school-key="${esc(schoolKey)}" data-note-scope="groups" data-note-key="${esc(groupKey)}" role="button" tabindex="0" aria-pressed="${active?'true':'false'}" aria-controls="${esc(g.id)}" aria-label="${esc(`${title}，${risk.riskText}${risk.probabilityText}，组内专业 ${g.majors.length} 个，${active?'当前已展开':'点击查看专业明细'}`)}"><div class="group-card-head"><h3>${groupTitleHTML(s,g)}${batchGuideBadgeHTML(s,g,true)}${noteBadge('groups',groupKey)}</h3><div class="group-card-status">${groupRiskHeroHTML(s,g,true)}<span class="group-card-state">${active?'当前查看':'点选查看'}</span></div></div>${cutoff26?`<div class="group-card-cutoff26">${cutoff26}</div>`:''}<div class="grid">${groupScoreMiniHTML(s,g)}<div class="mini"><b>${g.majors.length}</b><span>专业数</span></div></div><div class="tag-row">${groupCleanTagsHTML(s,g,planDiff,quality,true)}</div><div class="group-card-open"><span data-group-open-label>${active?'正在查看本组专业':'查看本组专业明细'}</span><b aria-hidden="true">${active?'↓':'→'}</b></div></article>`;
}
function groupSectionHTML(s,g,inlineChanges=false,activeGroupKey=null){
  const planDiff=(g.plan26||0)-(g.plan25||0);
  const quality=groupQuality(s,g);
  const title=groupDisplayTitleText(s,g);
  const risk=groupRiskState(s,g);
  const groupKey=keyGroup(s,g);
  const selectable=activeGroupKey!==null;
  const active=!selectable||groupKey===activeGroupKey;
  ...
  const cutoff26=inlineMajorCutoff26HTML(cardGroup);
  return `<section id="${g.id}" class="group-section group-quality-${quality.tone} group-risk-${risk.riskLevel} ${risk.selected?'selected-volunteer-group':''} ${selectable?'school-group-detail':''} ${selectable?(active?'is-active':'is-collapsed'):''}" data-group-key="${esc(groupKey)}" data-note-scope="groups" data-note-key="${esc(groupKey)}" aria-hidden="${selectable&&!active?'true':'false'}" title="${esc(`${quality.title}；${risk.title}`)}"><div class="group-head"><div class="group-head-main"><h3>${groupTitleHTML(s,g)}${batchGuideBadgeHTML(s,g)}${noteBadge('groups',groupKey)}</h3><p>${esc(s.name)}｜再选：${esc(g.requirement||'—')}｜${cutoff26?`${cutoff26}｜`:''}${groupScoreLineHTML(s,g)}｜26计划 ${fmt(g.plan26)}｜较25年 ${formatSigned(planDiff)} ${planDiff===0?'':`<span class="${signedClass(planDiff)}">(${formatSigned(planDiff)})</span>`}</p><div class="tag-row">${groupCleanTagsHTML(s,g,planDiff,quality,false)}</div><div class="anno-actions">${volunteerButtonHTML(s,g)}${batchGuideButtonsHTML(s,g)}${clearMajorButtonHTML(s,g)}<button class="anno-btn" data-annotation-scope="groups" data-annotation-key="${esc(groupKey)}" data-annotation-title="${esc(s.name)} ${esc(title)}｜专业组批注">查看批注</button><button class="anno-btn primary" data-annotation-scope="groups" data-annotation-key="${esc(groupKey)}" data-annotation-title="${esc(s.name)} ${esc(title)}｜专业组批注">新增批注</button></div></div><div class="group-head-side">${groupRiskHeroHTML(s,g)}${groupChangeButtonHTML(s,g,'section')}</div></div><div class="table-wrap"><table><thead><tr><th>专业志愿</th><th>代码</th><th>专业名称</th><th>专业类</th><th>26计划/变化</th><th>25分/位次</th><th>三年均分/位次</th></tr></thead><tbody>${currentRows}${deletedRows}</tbody></table></div></section>`;
}
...
function bindDynamic(){
  bindGroupCardSelectors();
  $('[data-print-school]')?.addEventListener('click',()=>window.print());
...
function activateGroupDetail(card,{scroll=true}={}){
  const schoolKey=card?.dataset.schoolKey||'';
  const groupKey=card?.dataset.groupSelect||'';
  if(!schoolKey||!groupKey)return;
  activeGroupKeyBySchool.set(schoolKey,groupKey);
  $$('.group-card-selector[data-group-select]').forEach(item=>{
    const active=item.dataset.schoolKey===schoolKey&&item.dataset.groupSelect===groupKey;
    item.classList.toggle('is-active',active);
    item.setAttribute('aria-pressed',active?'true':'false');
    const stateLabel=item.querySelector('.group-card-state');
    const openLabel=item.querySelector('[data-group-open-label]');
    const arrow=item.querySelector('.group-card-open b');
    if(stateLabel)stateLabel.textContent=active?'当前查看':'点选查看';
    if(openLabel)openLabel.textContent=active?'正在查看本组专业':'查看本组专业明细';
    if(arrow)arrow.textContent=active?'↓':'→';
  });
  let activeSection=null;
  $$('.school-group-detail[data-group-key]').forEach(section=>{
    const active=section.dataset.groupKey===groupKey;
  ...
  if(scroll&&activeSection){
    ...
    activeSection.scrollIntoView({behavior:reduced?'auto':'smooth',block:'start'});
  }
}
function bindGroupCardSelectors(){
  $$('.group-card-selector[data-group-select]').forEach(card=>{
    card.addEventListener('click',event=>{
      if(event.target.closest('button,a,input,select,textarea,label'))return;
      activateGroupDetail(card);
    });
...
function parentSchoolInfo(name){
  ...
  return rows.find(x=>normSchoolName(x.name)===target||((x.aliases||[]).some(a=>normSchoolName(a)===target)))||rows.find(x=>{
    const candidates=[normSchoolName(x.name),...(x.aliases||[]).map(normSchoolName)].filter(a=>a&&a.length>=6);
...
function parentKV(title,pairs){return infoSectionHTML(title,pairs,'parent-info-section');}
function parentSchoolInfoHTML(name,fallback={}){
  const info=parentSchoolInfo(name);
  const basic=info?.basic||info?.basicInfo||{};
  ...
  const net=info.network||info.networkAndElectricity||{};
  const daily=info.management||info.dailyManagement||{};
  const life=info.life||info.campusLife||{};
  const attentionNotes=evalInfo.notes||evalInfo.attentionNotes||[];
  ...
  return `<section class="info-section parent-info-section"><h4>位置与宿舍生活信息</h4><div class="parent-school-card"><p>${esc(basic.overview||'')}</p><dl class="kv info-kv">${[
    ...
    ['办学性质',info.ownership],
    ['城市层级',info.cityLevel],
    ['住宿判断',evalInfo.dormitory],
    ['交通便利度',evalInfo.transport],
    ['管理强度',evalInfo.management]
...
function selectableVolunteerMajors(g){
  return sortedMajors(g).filter(m=>!qualificationRiskBlocksSelection(qualificationRiskForMajor(null,g,m)));
}
...
function fillVolunteerFromCurrentFilters(){
  ...
  let picked=[];
  ['冲','稳','保'].forEach(name=>{
    picked=picked.concat(buckets[name].splice(0,Math.max(0,Math.min(plan[name],room-picked.length))));
  });
  ['稳','保','冲','待定'].forEach(name=>{
    while(picked.length<room&&buckets[name]?.length)picked.push(buckets[name].shift());
  });
  picked.forEach(item=>{
...
function volunteerMajorBadgesHTML(physical,subjectBlock,riskMeta,qualification){
  return `${medicalRestrictionLabelHTML(physical)}${subjectBlock?'<span class="risk-label">选科不符</span>':''}${qualificationRiskLabelHTML(qualification)}${majorRiskLabelHTML(riskMeta)}`;
}
function selectedVolunteerMajorHTML(s,g,m,key,index,total){
  ...
  return `<li class="volunteer-major-strip-item">
    ...
    <div class="volunteer-major-name">
      <b><span class="major-name-text">${esc(m.name)}</span>${majorMetaBadgesHTML(m,'四')}${volunteerMajorBadgesHTML(physical,'',riskMeta,qualification)}</b>
...
function showModuleChoiceModal(){
  ...
  $('#modal').innerHTML=`<h3>选择工作区</h3><div class="modal-body">
    <div class="account-notice"><b>登录成功。</b><br>请选择志愿填报、升学规划资讯、提前批、强基计划或综合评价工作区。</div>
    <div class="module-choice-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:12px;margin-top:14px">
      ...
      <button id="goStrongBaseModule" type="button" style="border:1px solid #d7cfbf;border-radius:18px;background:#fffaf0;color:#83542f;padding:16px;text-align:left;font-weight:900">强基计划助手<br><span style="display:block;margin-top:6px;color:#647067;font-size:12px;font-weight:600;line-height:1.5">完成学生画像、决策地图、39 校研究、报名提醒与面试训练。</span></button>
      <button id="goEarlyBatchModule" type="button" style="border:1px solid #d9c59b;border-radius:18px;background:#fff9e9;color:#79551c;padding:16px;text-align:left;font-weight:900">提前批助手<br><span style="display:block;margin-top:6px;color:#647067;font-size:12px;font-weight:600;line-height:1.5">完成资格初筛、通道决策、院校专业组对比与报考节点管理。</span></button>
    </div>
  ...
  $('#goStrongBaseModule')?.addEventListener('click',()=>{window.location.href='./strong-base/';});
  $('#goEarlyBatchModule')?.addEventListener('click',()=>{window.location.href='./early-batch/';});
}
...
function volunteerGroupsForAtomicSave(){
  return volunteerKeys.map((key,index)=>groupPayloadForSave(key,index)).filter(Boolean).map(row=>{
    const majors=majorPayloadsForSave(row.group_key,null).map(({form_group_id,owner_id,...major})=>major);
...
async function recordVolunteerExportIfPossible(rowCount,fileName=volunteerExportFileName()){
  ...
  try{
    await apiFetch('volunteer_exports',{method:'POST',body:JSON.stringify({form_id:currentVolunteerForm.id,student_id:currentStudent.id,owner_id:auth.user.id,format:'xls',file_name:fileName,group_count:volunteerKeys.length,major_count:Object.values(volunteerMajorKeys).reduce((sum,keys)=>sum+(Array.isArray(keys)?keys.length:0),0),source_payload:{rowCount}})});
