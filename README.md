...
- 将“专业组专业表”改为矩阵式结构：一行一个专业组，横向分为排序、院校专业组、已选专业、基础信息、排序与操作。
- 默认只显示已经填入的 1—6 个专业，不再把完整专业池摊开在主界面。
- 保留序号直改、拖动排序、上移/下移、删除、每组最多 6 个专业、专业按勾选顺序排序。
- 点击“编辑专业”或“一键展开”时，才展开完整专业池进行增减。
- 导出 Excel 改为一行一个专业组的志愿矩阵表。

## V1.1.31 行级权威数据重构版

- 志愿表专业组支持拖动排序：按住序号旁的 ↕ 拖动到目标位置。
- 志愿表专业组支持直接输入序号：点击当前序号，可输入 `10` 或 `第10`，自动跳转到对应位置。
- 保留 V1.1.31 行级权威数据重构版

## V1.1.31 行级权威数据重构版

- 对没有 2025 专业组投档分的 2026 新增组生成“预测分”，用于目标分区间筛选，避免新组被漏掉。
- 预测优先参考同校同批次中变迁来源组、同名/同类最相似专业；再按专业热度、专业单列、小计划、培养模式、中外合作/高成本、冷门/风险方向做加减分。
- 页面中预测值统一标为“预测分”或“估”，悬停可查看参考依据，不与真实 2025 分数混用。

## V1.1.31 行级权威数据重构版

- 志愿基础表所有导出单元格新增细边框，默认打开后就是完整表格线。
- 原“往年收分”拆为四列：25年分数、25年位次、3年平均分、3年平均位次。
- 分数、位次、招生人数均按数字写入，便于 Excel 里继续排序和筛选。

## V1.1.31 行级权威数据重构版

- 导出 Excel 改为单张“志愿基础表”，只保留院校代码、专业代码、专业组名称、专业名称、往年收分、招生人数、是否已选等核心字段。
- 同一专业组左侧单元格纵向合并并居中显示，右侧逐行列出该组全部专业。
- 用户勾选的专业标记为“已选”，未勾选专业标记为“未选”。
...
## V1.1.31 数据源重构说明
- 数据源改为《江苏高考专家版数据-已更新26最新招生计划（本科阶段汇总）-6.24更新.xlsx》的“汇总版”逐行读取。
- 每条专业记录以 Excel 原行作为事实源；今年计划、25/24/23分数位次均直接来自同一行。
- 专业唯一身份键：院校专业组代码 + 专业代码。已校验全表 22147 条记录无重复身份键。
- 不再因专业组编号变化跨组推断、覆盖或丢弃专业级历史分数。
